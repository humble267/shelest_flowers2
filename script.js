...
const buildProductCard = (product) => {
  ...
  const buyControl = isAvailable
    ? `<a class="catalog-product__button" href="${escapeHtml(getTelegramUrl(product))}" target="_blank" rel="noreferrer" data-product-id="${escapeHtml(product.id)}" data-product-name="${escapeHtml(product.name)}" data-product-image="${escapeHtml(product.image)}" data-product-price="${currentPrice}">Купити</a>`
  ...
  try {
    ...
    const getFilteredProducts = () => {
      let result =
        activeCategory === "all"
          ? [...allBouquetProducts]
          : allBouquetProducts.filter((p) => productHasCategory(p, activeCategory));
  ...
  try {
    ...
    const getFilteredToys = () => {
      ...
      if (activeCategory === "all") {
        result = [...allToyProducts];
      } else if (activeCategory === "sale") {
        result = visibleToys.filter((t) => t.isSale);
      } else {
        result = visibleToys.filter((t) => t.category === activeCategory);
      }
      if (sortOrder === "asc") result.sort((a, b) => a.price - b.price);
      else if (sortOrder === "desc") result.sort((a, b) => b.price - a.price);
      return result;
...
const renderBalloonsPage = async () => {
  ...
  try {
    ...
    const getFilteredBalloons = () => {
      let result;
      if (activeCategory === "all") {
        result = [...allBalloonProducts];
      } else if (activeCategory === "bd") {
        result = visibleBalloons.filter((b) => b.isForBD || b.category === "forbd");
      } else {
        result = visibleBalloons.filter((b) => b.category === activeCategory);
      }
      if (sortOrder === "asc") result.sort((a, b) => a.price - b.price);
      else if (sortOrder === "desc") result.sort((a, b) => b.price - a.price);
      return result;
...
const renderSalePage = async () => {
  ...
  try {
    ...
    const getFiltered = () => {
      let result;
      if (activeCategory === "flowers") result = [...saleFlowers];
      else if (activeCategory === "toys") result = [...saleToys];
      else if (activeCategory === "balloons") result = [...saleBalloons];
      else result = [...saleFlowers, ...saleToys, ...saleBalloons];
    ...
    const renderGrid = () => {
      ...
      contentEl.innerHTML = filtered.length
        ? `<div class="catalog-products__grid">${filtered.map(buildProductCard).join("")}</div>`
        : buildCatalogState("У цьому розділі поки що немає акційних товарів.");
      bindPageTransitions(contentEl);
      applyImageFallbacks(contentEl);
    };
